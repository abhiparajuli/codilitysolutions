...
class Solution {
    public int solution(int N) {
        ...
        int start = 0;
        int gap = 0;
        int binNumL = binNum.Length;
        Console.WriteLine($"Bin Input: {binNum}");

        for(int i=1; i < binNumL; i++){
            if(binNum[i]=='1'){
                int gapNew = i - start - 1;
                start = i;
                if(gapNew > gap){
                    gap = gapNew;
                }
                Console.WriteLine($"New Gap: {gapNew}");              
            }
        }
        Console.WriteLine($"Final Gap: {gap}");
        return gap;
    }
}
